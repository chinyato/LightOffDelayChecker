...
void __fastcall TForm1::FormCreate(TObject *Sender)
{
	...
	if( DirectoryExists( uStr )) {
		...
		if( ComboBox2->ItemIndex >= 0 ) {
			Edit1->Text = uStr + L"\\" + ComboBox2->Items->Strings[ ComboBox2->ItemIndex ];
			uStr = System::Ioutils::TPath::GetCachePath() + L"\\" + ComboBox2->Items->Strings[ ComboBox2->ItemIndex ];
		} else {
			Edit1->Text = uStr + L"\\ChiTuBox";
			uStr = System::Ioutils::TPath::GetCachePath();
		}
		Edit2->Text = uStr  + L"\\machine" + L"\\1.cfg";
...
void __fastcall TForm1::ComboBox1Change(TObject *Sender)
{
	...
	if( ComboBox2->ItemIndex >= 0 ) {
		Edit1->Text = exePath + L"\\" + ComboBox2->Items->Strings[ ComboBox2->ItemIndex ];
	} else {
		Edit1->Text = exePath + L"\\ChiTuBox";
	}
	Edit2->Text = Edit1->Text + L"\\machine" + L"\\1.cfg";
...
int __fastcall TForm1::LoadFromLocalAppData( void )
{
	...
	//2021/01/05
	uStrFileName = Edit1->Text +  L"\\machine\\1.cfg";
/*
	if( ComboBox2->ItemIndex >= 0 ) {
		uStrFileName = exePath + ComboBox2->Items->Strings[ ComboBox2->ItemIndex ] +  L"\\machine\\1.cfg";
	} else {
		uStrFileName = exePath + L"\\ChiTuBox\\machine\\1.cfg";
	}
*/
...
void __fastcall TForm1::CheckBox1Click(TObject *Sender)
{
	...
	if( CheckBox1->Checked ) {
		...
		//2021/01/05
		Label17->Caption = L"Save Data Folder Name";
